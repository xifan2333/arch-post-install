...
function parseConfig(rawText) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    switch (key) {
      case "device": cfg.device = val || "auto"; break
      case "width": cfg.width = Math.max(80, Number(val) || cfg.width); break
      case "height": cfg.height = Math.max(60, Number(val) || cfg.height); break
      case "capture_width": cfg.captureWidth = Math.max(160, Number(val) || cfg.captureWidth); break
      case "capture_height": cfg.captureHeight = Math.max(120, Number(val) || cfg.captureHeight); break
      case "fps": cfg.fps = Math.max(1, Number(val) || cfg.fps); break
...
function serializeConfig(cfg) {
  const lines = [
    ...
    "capture_width = " + (Math.round(cfg.captureWidth) || 1280),
    "capture_height = " + (Math.round(cfg.captureHeight) || 720),
...
function pickDevice(inputs, want) {
  ...
  if (needle && needle !== "auto") {
    for (let i = 0; i < inputs.length; i++) {
      ...
      if (id === needle || id.indexOf(needle) !== -1 || desc.indexOf(needle) !== -1)
        return d
    }
  }
  let best = inputs[0]
