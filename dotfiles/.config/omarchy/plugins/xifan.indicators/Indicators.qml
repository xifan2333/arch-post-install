...
BarWidget {
  ...
  function indicatorEntriesFromSettings(settings) {
    ...
    else if (settings.indicators && typeof settings.indicators.length === "number" && settings.indicators.length > 0)
  ...
  function activeModelIndex(id) {
    for (var i = 0; i < activeIndicatorModel.count; i++) {
      if (activeIndicatorModel.get(i).activeId === id)
        return i;
    }
    return -1;
  }
  ...
  function copyActiveStates() {
    ...
    for (var id in indicatorActiveStates) {
      if (indicatorActiveStates[id] === true)
        states[id] = true;
    }
    return states;
  }
  ...
  function syncActiveIndicatorModel() {
    ...
    for (var j = 0; j < activeIndicatorIds.length; j++) {
      ...
      if (index === -1)
        activeIndicatorModel.insert(j, {
          activeId: id
        });
      else if (index !== j)
        activeIndicatorModel.move(index, j, 1);
    }
  }
  ...
  implicitWidth: root.vertical ? Math.max(activeVerticalBlock.implicitWidth, inactiveVerticalArea.implicitWidth) : activeHorizontalBlock.implicitWidth + inactiveHorizontalArea.implicitWidth
  implicitHeight: root.vertical ? activeVerticalBlock.implicitHeight + inactiveVerticalArea.implicitHeight : Math.max(activeHorizontalBlock.implicitHeight, inactiveHorizontalArea.implicitHeight)
  ...
  component IndicatorLoader: Item {
    ...
    Loader {
      ...
      onStatusChanged: if (status === Loader.Error)
        console.warn("Indicator loader error", indicatorSlot.indicatorId, source)
    }
    ...
    function injectProps() {
      ...
      if ("moduleName" in target)
        target.moduleName = indicatorId;
      if ("settings" in target)
        target.settings = indicatorSettings;
      if ("indicatorBlock" in target)
        target.indicatorBlock = indicatorBlock;
      if ("indicatorHost" in target)
        target.indicatorHost = root;
      if ("activeOverride" in target)
        target.activeOverride = indicatorBlock === "active" ? true : null;
    }

    function syncActiveState() {
      if (!reportActiveState || !indicatorsModule || !indicatorsModule.setIndicatorActive)
        return;
      var active = !!indicatorSource.item && indicatorSource.item.active === true;
      if (indicatorBlock === "active") {
        if (active)
          activeStateObserved = true;
        else if (!activeStateObserved)
          return;
      }
