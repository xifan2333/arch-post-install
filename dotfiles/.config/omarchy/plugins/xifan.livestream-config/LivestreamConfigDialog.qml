...
Item {
  ...
  FileView {
    ...
    watchChanges: true
    printErrors: false
    onLoaded: if (!root.opened)
      root.loadConfig(text())
    onFileChanged: if (!root.opened)
      reload()
  }
  ...
  function saveConfig() {
    ...
    Quickshell.execDetached(["bash", "-c", "mkdir -p \"$(dirname \"$2\")\" && chmod 700 \"$(dirname \"$2\")\" 2>/dev/null && printf '%s' \"$1\" > \"$2\" && chmod 600 \"$2\"", "--", serialized, root.configPath]);
  }
  ...
  function removePlatform(index) {
    if (index < 0 || index >= root.platforms.length)
      return;
    const copy = root.platforms.slice();
    copy.splice(index, 1);
    root.platforms = copy;
